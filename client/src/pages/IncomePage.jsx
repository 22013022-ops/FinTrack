...
export default function IncomePage() {
  ...
  const save = async (values, id) => {
    ...
    if (id) await incomeService.update(id, values);
    else await incomeService.create(values);

    setModal(null);
    setToast({ type: 'success', text: id ? 'Income updated successfully.' : 'Income added successfully.' });
    load();
  };
  ...
  const remove = async (id) => {
    try {
      ...
      setToast({ type: 'error', text: error.response?.data?.message || 'Could not delete income.' });
    }
  };
    ...
    <div className="income-page">
      <header className="income-header">
        <div>
          ...
          <h1>Income</h1>
          <p>Track every source of income, all in one place.</p>
        </div>
      </header>
      ...
      <section className="income-panel">
        ...
        {loading ? (
          ...
          <EmptyMonth month={month} onAdd={() => setModal({ type: 'form', income: null })} />
        ) : isNoMatch ? (
                    ...
                    <tr key={record._id}>
                      ...
                      <td>
                        <div className="table-actions">
                          <button onClick={() => setModal({ type: 'form', income: { ...record, date: record.date.slice(0, 10) } })} aria-label="Edit income"><Edit3 size={16} /></button>
                          <button className="delete-button" onClick={() => setModal({ type: 'delete', income: record })} aria-label="Delete income"><Trash2 size={16} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {modal?.type === 'delete' && <DeleteModal income={modal.income} onClose={() => setModal(null)} onConfirm={() => remove(modal.income._id)} />}
      {toast && (
...
function EmptyMonth({ month, onAdd }) {
  return (
    <div className="empty-state">
      ...
      <h3>You're yet to add income for this month.</h3>
      <p>No income has been recorded for {getMonthLabel(month)} yet.</p>
      <Button onClick={onAdd}><CirclePlus size={18} /> Add income</Button>
    </div>
  );
}
...
function FilterMenu({ activeView, setActiveView, filters, onChange, onApply, onClose }) {
  ...
  return <div className="filter-menu"><aside>{items.map(([key, label, Icon]) => <button type="button" key={key} onClick={() => setActiveView(key)} className={activeView === key ? 'active' : ''}><Icon size={18}/><span>{label}</span>{(key === 'categories' && filters.categories.length) || (key === 'description' && filters.description) || (key === 'amount' && (filters.amountMin || filters.amountMax)) || (key === 'date' && (filters.dateFrom || filters.dateTo)) ? <i>✓</i> : null}</button>)}</aside><div className="filter-menu-content"><header><div><p className="eyebrow">FILTER INCOME</p><h3>{title}</h3></div><button type="button" onClick={onClose}><X size={18}/></button></header>{activeView === 'categories' && <div className="filter-category-list"><button type="button" className={!filters.categories.length ? 'selected' : ''} onClick={() => onChange('categories', [])}>All categories</button>{categories.map((category) => <button type="button" key={category} className={filters.categories.includes(category) ? 'selected' : ''} onClick={() => onChange('categories', filters.categories.includes(category) ? filters.categories.filter((item) => item !== category) : [...filters.categories, category])}><span>{category}</span>{filters.categories.includes(category) && <b>✓</b>}</button>)}</div>}{activeView === 'description' && <label className="menu-field">Description<input autoFocus value={filters.description} onChange={(e) => onChange('description', e.target.value)} placeholder="Match description" /></label>}{activeView === 'amount' && <div className="menu-field-grid"><label className="menu-field">Min amount<input type="number" min="0" value={filters.amountMin} onChange={(e) => onChange('amountMin', e.target.value)} placeholder="0" /></label><label className="menu-field">Max amount<input type="number" min="0" value={filters.amountMax} onChange={(e) => onChange('amountMax', e.target.value)} placeholder="No limit" /></label></div>}{activeView === 'date' && <div className="menu-field-grid"><label className="menu-field">From<input type="date" value={filters.dateFrom} onChange={(e) => onChange('dateFrom', e.target.value)} /></label><label className="menu-field">To<input type="date" value={filters.dateTo} onChange={(e) => onChange('dateTo', e.target.value)} /></label></div>}<footer><button type="button" className="text-button" onClick={onClose}>Cancel</button><Button type="button" onClick={onApply}>Apply filters</Button></footer></div></div>;
}

function IncomeModal({ income, onClose, onSave }) {
  const [values, setValues] = useState(income || initialForm());
  ...
  const submit = async (e) => {
    ...
    setError('');
    try {
      await onSave(values, income?._id);
    } catch (requestError) {
      setError(requestError.response?.data?.message || requestError.message || 'Unable to save income.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true">
      <form className="income-modal" onSubmit={submit}>
        <header>
          <div>
            <p className="eyebrow">{income ? 'EDIT RECORD' : 'NEW RECORD'}</p>
        ...
        <footer>
          ...
          <Button type="submit" disabled={saving}>{saving ? 'Saving…' : income ? 'Save changes' : 'Add income'}</Button>
        </footer>
      </form>
    </div>
  );
...
function DeleteModal({ income, onClose, onConfirm }) {
  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true">
      <div className="confirm-modal">
        <div className="delete-icon"><Trash2 size={21} /></div>
        <h2>Delete this income?</h2>
        <p>This will permanently remove the {formatCurrency(income.amount)} {income.category.toLowerCase()} record.</p>
        <div>
...
function LoadingRows() {
  return (
    <div className="income-loading">
      ...
      <span>Fetching your income activity…</span>
    </div>
  );
}
